...
import { StartupError } from "../src/errors";
...
test("runtime toggles readiness during start and stop", async (t) => {
  ...
  assert.equal(runtime.readiness().phase, "starting");

  await runtime.start();
  assert.equal(runtime.readiness().phase, "ready");
  ...
  await runtime.stop();
  assert.equal(runtime.readiness().phase, "stopping");
...
test("runtime startup fails when the database cannot be created", async () => {
  ...
  await assert.rejects(
    ...
    (error) =>
      ...
      error.message === "Failed to initialize SQL." &&
      error.cause instanceof Error &&
      error.cause.message === "connect failed",
  );
});
