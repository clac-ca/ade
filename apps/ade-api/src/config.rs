...
mod tests {
    ...
    #[test]
    fn requires_sql_connection_string() {
        let error = AppConfig::from_env(&env(&[])).unwrap_err();
    ...
    fn reads_trimmed_sql_when_present() {
        let config =
            AppConfig::from_env(&env(&[("AZURE_SQL_CONNECTIONSTRING", " Server=sql; ")])).unwrap();

        assert_eq!(config.sql_connection_string, "Server=sql;");
    }
}
