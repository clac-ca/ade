import {
  ...
  parseSqlConnectionString as parseTediousSqlConnectionString,
} from "@tediousjs/connection-string";
...
type ConnectToDatabaseLike = (
  connectionString: string,
) => Promise<SqlBatchConnection>;
type ConnectionFields = Record<
  string,
...
function parseServer(value: string): {
  ...
  if (trimmed === "") {
    throw new ConfigError("SQL connection string must include a server.");
  }
  ...
  if (!/^[1-9]\d*$/.test(portValue)) {
    throw new ConfigError(`Invalid SQL Server port: ${portValue}`);
  }
...
function readOptionalStringField(
  ...
  if (typeof value !== "string") {
    throw new ConfigError(
      `SQL connection string field "${name}" must be a string.`,
    );
  }
...
function readRequiredField(fields: ConnectionFields, keys: string[]): string {
  ...
  throw new ConfigError(
    `SQL connection string is missing one of: ${keys.join(", ")}`,
  );
}
...
function readBooleanField(
  ...
  throw new ConfigError(
    `Invalid boolean value in SQL connection string field "${name}": ${String(value)}`,
  );
}
...
function parseAuthentication(fields: ConnectionFields) {
  ...
  throw new ConfigError(
    `Unsupported SQL authentication mode: ${authentication}`,
  );
}

function parseSqlConnectionString(
  connectionString: string,
): ParsedSqlConnectionString {
  const fields = parseConnectionFields(connectionString);
  const { port, server } = parseServer(
    readRequiredField(fields, ["data source"]),
  );
  ...
  const encrypt = readBooleanField(
    fields,
    "encrypt",
...
async function ensureDatabaseExists(
  connectionString: string,
  connect: ConnectToDatabaseLike = connectToSql,
): Promise<void> {
  const parsed = parseSqlConnectionString(connectionString);
...
export {
  buildPoolConfig,
  connectToSql as connectToDatabase,
