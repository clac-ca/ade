...
| `AZURE_SQL_CONNECTIONSTRING`         | Yes         | `Server=host.docker.internal,8013;Database=ade;User Id=sa;Password=<LOCAL_SQL_PASSWORD>;Encrypt=false;TrustServerCertificate=true` | API, migrate job  | Required at runtime. App startup fails fast if SQL is missing or unreachable.                             |
...
The `.env.example` values are examples for `pnpm start`, not for `pnpm dev`.
...
`pnpm start` is production-like and requires a reachable SQL dependency. It does not start local dependencies for you.

When `pnpm start` connects to local services running on the host, use `host.docker.internal` in connection strings rather than `127.0.0.1`.
